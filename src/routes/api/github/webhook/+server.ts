...
/**
 * Receive GitHub App webhooks and dispatch them through Probot.
 *
 * The raw request body is forwarded so Octokit can verify the HMAC
 * signature. Failures return JSON with an explicit status instead of
 * throwing, which keeps SvelteKit from leaking stack traces to GitHub's
 * delivery log while still surfacing 4xx/5xx for retries.
 */
