	...
	switch (action) {
		...
		case "synchronize": {
			...
			const contentFiles = files.filter((f) =>
				f.filename.startsWith(contentPrefix),
			);
			const summary = [
				...
				...contentFiles.map((f) => {
					const status =
						f.status === "added" ? "🆕" : f.status === "removed" ? "🗑️" : "✏️";
			...
			const botComment = comments.find(
				(c) =>
					c.user?.type === "Bot" && c.body?.includes("LightCMS Content Review"),
			);
			...
			// Add a label
			const { data: labels } =
				await context.octokit.rest.issues.listLabelsOnIssue({
					owner,
					repo,
					issue_number: prNumber,
				});
