...
export function assertContentPath(
	path: string,
...
export function ensureUnderContentRoot(
...
export function resolveContentPath(
	...
	const parts = segments
		.map((s) => normalizeContentPath(s))
		.filter((s) => s.length > 0);
	if (parts.length === 0) return CONTENT_ROOT;
	const joined = parts.join("/");
	if (joined === CONTENT_ROOT || joined.startsWith(`${CONTENT_ROOT}/`))
		return joined;
