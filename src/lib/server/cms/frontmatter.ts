		...
		if (value instanceof Date) {
			...
			rest[key] = value.map((item) =>
				item instanceof Date
					? item.toISOString().replace(/\.\d{3}Z$/, "Z")
					: item,
			);
		}
	}
...
export function assertSlug(slug: string): void {
	if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) {
		throw new Error(
			"A slug must contain lowercase letters, numbers, and hyphens only.",
		);
	}
}
...
function formatDatetimeToUtc(value: string): string {
	...
	if (!iso8601Pattern.test(trimmed)) {
		throw new Error(
			`Invalid datetime — expected ISO 8601 (YYYY-MM-DDTHH:mm:ssZ), got "${value}"`,
		);
	}
