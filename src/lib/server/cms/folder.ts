...
export async function createFolderAtPath(
	...
	const message =
		opts?.message ?? `Create folder ${normalizedName} at ${normalizedParent}`;
...
export async function deleteFolderAtPath(
	...
	try {
		...
		throw new Error(
			`Folder not found: ${cause instanceof Error ? cause.message : String(cause)}`,
		);
	}
...
export async function deleteCollectionFolder(
	...
	if (fullPath === CONTENT_ROOT || fullPath === collection.path) {
		throw new Error(
			"Cannot delete the collection root. Remove individual subfolders instead.",
		);
	}
	const message =
		opts?.message ??
		`Delete folder ${normalizedFolder} in ${collection.label ?? collection.name}`;
