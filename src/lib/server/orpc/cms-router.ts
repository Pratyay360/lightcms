...
async function resolveCollectionCtx(
	sessionUserId: string,
	input: z.infer<typeof repoInput>,
) {
...
const getEntry = authedProcedure
	...
	.handler(async ({ input, context }) => {
		const { collection, ctx } = await resolveCollectionCtx(
 ...
 */ const createFolder = authedProcedure
	.input(folderInput)
	.handler(async ({ input, context }) => {
 ...
 */ const createFile = authedProcedure
	.input(fileInput)
	.handler(async ({ input, context }) => {
		const ctx = await resolveCtx(context.session.userId, input);
		return orpcTry(
...
const initializeDir = authedProcedure
	.input(dirInput)
	.handler(async ({ input, context }) => {
		const ctx = await resolveCtx(context.session.userId, input);
		return orpcTry(
...
const deleteFolder = authedProcedure
	.input(dirInput)
	.handler(async ({ input, context }) => {
		const ctx = await resolveCtx(context.session.userId, input);
		return orpcTry(
