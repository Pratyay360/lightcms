			...
			new Plugin({
				...
				state: {
					...
					apply(tr, prev, _oldState, newState) {
						...
						if (inlineMap[tail])
							inlineGhost = inlineMap[tail].slice(tail.length);
						else if (textBefore.endsWith("[")) inlineGhost = "](url)";

						const found = getSuggestion(trimmed);
						if (!found && !inlineGhost)
							return { deco: DecorationSet.empty, active: null };

						const pos = sel.$from.pos;
						const ghost = found
							? found.suggestion.ghost(found.match)
							: inlineGhost!;
						...
						return {
							...
							active: found
								? { suggestion: found.suggestion, match: found.match, pos }
								: null,
						};
					},
				},
				props: {
					...
					handleKeyDown(view, e) {
						...
						editor
							.chain()
							.focus()
							.deleteRange({ from: parentStart, to: from })
							.run();
