 ...
 */
export function createGripInteractionPlugin(
	opts: GripInteractionOptions,
): Plugin {
	...
	function updateGripVisibility(table: HTMLTableElement, activeIndex: number) {
		const grips = table.querySelectorAll<HTMLAnchorElement>(
			`a.${gripSelector}`,
		);
		grips.forEach((g, idx) => {
		...
		props: {
			handleDOMEvents: {
				...
				mouseleave: (_view, event) => {
					const table = (event.target as HTMLElement).closest(
						"table",
