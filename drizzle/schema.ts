import { pgTable, uniqueIndex, index, foreignKey, text, timestamp, unique, boolean, integer, primaryKey, bigint } from "drizzle-orm/pg-core"
...
export const githubInstallation = pgTable("github_installation", {
	...
	index("github_installation_userId_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
			name: "github_installation_user_id_user_id_fk"
		}).onDelete("cascade"),
	primaryKey({ columns: [table.userId, table.installationId], name: "github_installation_user_id_installation_id_pk"}),
]);
